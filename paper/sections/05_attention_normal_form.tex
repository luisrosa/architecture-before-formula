...
The single factor $\kappa_{ri}$ contains every state-independent positive pair term that is extensionally relevant to the row. It may be decomposed into schema-, instance-, or parameter-origin factors only when those factors are independently marked. A finite additive logit bias is equivalent to multiplying $\kappa_{ri}$ by its exponential and is not counted twice.

\begin{assumption}[Compositional positive link]\label{ass:link}
On every admissible pair, the positive raw weight is linked to the marked baseline and scalar evidence by
\[
w_{ri}(H):=\kappa_{ri}\psi\!\left(s_{ri}(H)\right),
\]
where
\[
\psi:\R\to(0,\infty)
...
\psi(a+b)=\psi(a)\psi(b).
\]
Thus independent additive evidence increments combine multiplicatively in positive raw support.
\end{assumption}

\begin{assumption}[Receiver-row anchoring]\label{ass:row-anchor}
For each receiver $r$, the positive raw weights $(w_{ri})_{i\in S_r}$ are converted to coefficients $(\alpha_{ri})_{i\in S}$ such that:
...
\begin{proof}
Fix $j_0\in A$ and set $c:=\alpha_{j_0}/w_{j_0}>0$. Ratio preservation gives $\alpha_i=cw_i$ for every $i\in A$ (with the $i=j_0$ case immediate). Unit mass determines $c=(\sum_jw_j)^{-1}$.
\end{proof}
...
\begin{proof}
The compositional-link assumption and exponential-link lemma give positive raw weights $\kappa_{ri}e^{s_{ri}(H)/\tau}$. The separation-rank proposition gives $s_{ri}(H)=\ip{q(x_r(H))}{k(y_i(H))}$. The row-anchor lemma gives \eqref{eq:attention-weights}, and source-local payloads with additive aggregation give \eqref{eq:attention-normal-form}.
\end{proof}
...
v_i(h)=W_Vh,
\]
and a finite supplied additive logit bias $b_{ri}$ is represented by $\kappa_{ri}=e^{b_{ri}}$ on admissible pairs, then \Cref{thm:attention-normal-form} is masked scaled dot-product attention after choosing the conventional score scale (for example, $\tau=\sqrt d$ when the dot product is otherwise unscaled) \citep{vaswani2017attention}.
\end{corollary}

If a supplied bias is instead parameterized in the same pre-temperature evidence units as $s_{ri}$, say $\widetilde b_{ri}=\tau b_{ri}$, the equivalent baseline is $\kappa_{ri}=e^{\widetilde b_{ri}/\tau}$. A state-dependent pair bias should instead remain in $s_{ri}(H)$. The decomposition between $\kappa$ and $s$ is provenance-relative: it is meaningful when the architecture marks which term is state-independent and which is constructed from current state.
...
\begin{proof}
The layer uses finite bilinear scores on admissible pairs, positive exponentiation, rowwise normalization, source-local values, and additive aggregation. A finite additive logit bias can be represented by $\kappa$; forbidden pairs are represented by $M=0$.
\end{proof}
...
\[
\ip{q'(x)}{k'(y)}
=q(x)^{\mathsf T}R^{\mathsf T}R^{-\mathsf T}k(y)
=q(x)^{\mathsf T}k(y).
\]
\end{proof}
